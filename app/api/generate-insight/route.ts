...
import { insightRatelimit } from "@/lib/redis"
import { withRateLimit } from "@/lib/rate-limit-middleware"

export async function POST(req: Request) {
  console.log("[v0] API generate-insight chamada")

  return withRateLimit(
    req as any,
    {
      ratelimit: insightRatelimit,
      fallbackResponse: {
        insight: "Muitas requisições em pouco tempo. Aguarde alguns segundos antes de gerar novos insights.",
        fallback: true,
        rateLimited: true,
      },
    },
    async () => {
      console.log("[v0] Passou pelo rate limit, processando requisição")
      ...
      if (!title || !perspective) {
        console.log("[v0] Faltam campos obrigatórios")
      ...
      try {
        console.log("[v0] Tentando gerar insight com IA...")
...
- Entre 450-500 caracteres (incluindo espaços)`
        ...
        console.log("[v0] Insight gerado com sucesso pela IA, tamanho:", truncatedInsight.length)

        return Response.json({ insight: truncatedInsight })
      } catch (aiError) {
        console.log("[v0] Erro ao chamar IA:", aiError instanceof Error ? aiError.message : String(aiError))
        console.log("[v0] Usando fallback dinâmico")
